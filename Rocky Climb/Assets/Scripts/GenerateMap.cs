...
public class GenerateMap : MonoBehaviour {
    ...
    void Start () {
        Destroy(Instantiate(rocksPrefabs[rocksPrefabs.Length -1],rightHand.position,Quaternion.identity),7f); //создание первого камня
    ...
    public void CreateNewPos() //Метод для создания камня
    {
        ...
        Destroy(Instantiate(rocksPrefabs[rnd2],position,Quaternion.identity),20f); //Непосредственно создание камня
        if(rnd3 == 2)
        {
